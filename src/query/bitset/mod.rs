...
/// Backing store for a [`BitSetDocSet`].
///
/// Implemented for both the in-memory [`BitSet`] and the serialized
/// [`ReadOnlyBitSet`], so a `BitSetDocSet` can iterate over either without
/// materializing the latter back into a `BitSet`.
